...
const validateVideoPostRequest = async (req, res, next) => {
  try {
    ...
    const { title, description, link, thumbnail, category, private } = req.body
    ...
    req.video = { channelId: user.channel._id, title, description, thumbnail, category, link, private }
    next()
  } catch (error) {
    next(error)
  }
